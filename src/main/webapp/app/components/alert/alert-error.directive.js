(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('ascensionApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-ascensionApp-error');
                var entityKey = httpResponse.headers('X-ascensionApp-params');
                if (errorHeader) {
